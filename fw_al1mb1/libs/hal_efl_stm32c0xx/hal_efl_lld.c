...
/**
 * @file    hal_efl_lld.c
 * @brief   STM32C0xx Embedded Flash subsystem low level driver source.
...
/* The descriptor for 32K devices (C011, C031, C051). */
static const flash_descriptor_t efl_lld_size_32k[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank. */
       ...
       .sectors_count     = STM32_FLASH_SECTORS_TOTAL_32K,
       .sectors           = NULL,
       .sectors_size      = STM32_FLASH_SECTOR_SIZE,
       .address           = (uint8_t *)FLASH_BASE,
       .size              = STM32_FLASH_SIZE_32K * STM32_FLASH_SIZE_SCALE
      }
};

/* The descriptor for 64K devices (C071 variant). */
static const flash_descriptor_t efl_lld_size_64k[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED |
                            FLASH_ATTR_ECC_CAPABLE   |
...
/* The descriptor for 128K devices (C071 variant). */
static const flash_descriptor_t efl_lld_size_128k[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED |
                            FLASH_ATTR_ECC_CAPABLE   |
...
/* The descriptor for 256K devices (C091, C092). */
static const flash_descriptor_t efl_lld_size_256k[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED |
                            FLASH_ATTR_ECC_CAPABLE   |
...
static const efl_lld_size_t efl_lld_flash_sizes[] = {
      {
       .desc = efl_lld_size_32k
      },
      {
       .desc = efl_lld_size_64k
      },
      {
       .desc = efl_lld_size_128k
      },
      {
...
static inline bool stm32_flash_dual_bank(EFlashDriver *eflp) {
  ...
  /* STM32C0xx has only single bank flash - no dual bank support. */
  (void)eflp;
  return false;
}
 ...
 * @retval                          Pointer to single bank descriptor.
 *
 * @notapi
 */
...
flash_error_t efl_lld_program(void *instance, flash_offset_t offset,
  ...
  while (n > 0U) {
    ...
    address = (volatile uint32_t *)(volatile void *)(bank->address +
 ...
 * @note    Mass erase is not supported on STM32C0xx single-bank devices.
...
flash_error_t efl_lld_start_erase_all(void *instance) {
  ...
  /* Mass erase not allowed on single bank devices. */
...
flash_error_t efl_lld_query_erase(void *instance, uint32_t *msec) {
  ...
  if (devp->state == FLASH_ERASE) {
    ...
    if ((devp->flash->SR & FLASH_SR_BSY1) == 0U) {
      ...
      devp->flash->CR &= ~(FLASH_CR_MER1 | FLASH_CR_PER);
...
flash_error_t efl_lld_verify_erase(void *instance, flash_sector_t sector) {
  ...
  address = (uint32_t *)(void *)(bank->address +
